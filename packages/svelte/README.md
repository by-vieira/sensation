...
`@morgan-vieira-npm/sensation-svelte` provides the Svelte theme context and the first RbxSensation foundation components.
...
`ThemeDepth` changes the context depth without drawing an element. `getThemeState` returns the current reactive theme state during component initialization.

## Text

`Text` renders a `<span>`. It accepts native span attributes. Bind `element` to access the span.
...
`Panel` renders a `<div>` and adds no padding. It accepts native div attributes. Bind `element` to access the div.
...
`Button` renders a native `<button>`. Native attributes, form behavior, and events pass through. Bind `element` to access the button.
...
The browser measures and wraps text. Bind `element` and use `ResizeObserver` when an application needs measured output.
