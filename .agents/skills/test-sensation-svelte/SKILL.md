---
name: test-sensation-svelte
description: Launch, retain, and test the Sensation Svelte workbench, including worktree-safe Vite sessions, controlled browser interaction, component-state fixtures, visual evidence, and focused package verification. Use after Svelte component changes, when reproducing Svelte behavior, or when handing an interactive workbench to the user.
---

# Test Sensation Svelte

Use this skill for `@morgan-vieira-npm/sensation-svelte`. For the React implementation, use the sibling [`test-sensation-react`](../test-sensation-react/SKILL.md) skill.

## Start or reuse the Svelte workbench

1. Run commands from the repository root.
2. Check for an existing Svelte Vite process and browser tab from the current testing loop. Reuse them when they belong to this worktree and still load the current source.
3. If dependencies are unavailable, run `pnpm install --frozen-lockfile`. Do not reinstall dependencies for every test pass.
4. Start the workbench with `pnpm dev:svelte`. Keep the terminal session alive and read the actual local URL from Vite's output.
...
Use `apps/svelte-demo` as the test harness. If it does not expose the changed behavior, add the smallest fixture that makes the relevant states and transitions reachable. Keep a fixture when it remains useful as a component example or regression case. Otherwise remove only the temporary fixture changes that you made.
...
Use semantic browser locators and current page state instead of fixed coordinates. Exercise mouse, keyboard, touch emulation, and assistive semantics where each input applies. Check native element behavior, visible focus, forms, prop and event forwarding, reactive updates, rapid repeated input, cleanup after unmounting, narrow layouts, increased text size, and reduced motion.
...
```powershell
pnpm exec oxfmt --check packages/svelte apps/svelte-demo
pnpm exec oxlint packages/svelte apps/svelte-demo
pnpm --filter @morgan-vieira-npm/sensation-svelte check
pnpm --filter @morgan-vieira-npm/sensation-svelte build
pnpm --filter @morgan-vieira-npm/sensation-svelte-demo check
pnpm --filter @morgan-vieira-npm/sensation-svelte-demo build
```
...
After interrupting the terminal session, verify that the selected port no longer has a listener. On Windows, the pnpm parent can exit while Vite remains alive. If that happens, inspect the listener's command line and stop it only when it points to this worktree's `apps/svelte-demo` directory.
...
- If reactive state appears stale, verify that the browser loaded the current worktree before diagnosing component ownership or cleanup.
